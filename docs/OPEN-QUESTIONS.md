...
| Q-H12 | Quality bar and working style | ✅ **Readable code is the goal**, and Biome also blocks functions above cognitive complexity 15. Prefer a mature, maintained tool to re-implementing one; logic is covered by tests; a change is only done after it is verified in practice (tests, `--dry-run`, CI), with the output shown. Before a non-trivial change, show what was understood and wait for confirmation; when a request is ambiguous, offer concrete options instead of guessing. Public metadata (the npm and GitHub descriptions) is short, without optional details |
