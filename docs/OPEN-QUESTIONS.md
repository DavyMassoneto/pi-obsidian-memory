...
| Q-H10 | Tipos exatos na borda | ✅ **Proibidos** (o Biome barra, no código e nos testes): comparação com `undefined`/`null` e conversão manual para boolean. Dado externo (argv, saída de comando, regex, JSON) passa uma vez por schema do TypeBox (`parseValue`); operação que pode falhar devolve resultado explícito (`{ succeeded, output }`); verificação devolve lista de problemas. As flags voltam a ter `default: false` no `parseArgs` e o `--bump` volta a `auto` (padrão do próprio git-cliff): default que é parte da definição da CLI é necessário; o que é proibido é default escondido na lógica |
| Q-H9 | Ternários e defaults | ✅ **Proibidos**, no código e nos testes: ternário, `??`, `\|\|` que devolve valor e valor default em parâmetro ou desestruturação (o Biome barra); dado opcional sem necessidade também não entra. Cada caso vira `if` com retorno explícito, e dado obrigatório que falta vira erro, nunca um valor inventado. Consequências no release: `package-lock.json` e `tagprefix` obrigatórios. (O `default:` das flags voltou na Q-H10) |
| Q-H8 | Barrels e imports | ✅ **Toda pasta de código tem um `index.ts`** só com `export *` (arquivos da pasta e `index.ts` das subpastas). Import da mesma pasta: o arquivo direto (`./x.ts`); de outra pasta: o barrel mais alto ao alcance (`../index.ts`); `../../` proibido em qualquer arquivo. Testes importam por `#scripts` (subpath import do Node). Funções no topo com `function`, por causa dos imports circulares dos barrels. Entrada do release em `scripts/release/run.ts`, fora dos barrels. O Biome barra os imports e o conteúdo dos `index.ts`; o `tests/barrels.test.ts` confere se toda pasta tem o seu |
