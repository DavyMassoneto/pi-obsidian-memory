...
| Q-H9 | Ternários e defaults | ✅ **Proibidos**, no código e nos testes: ternário, `??`, `\|\|` que devolve valor e valor default em parâmetro ou desestruturação (o Biome barra); `default:` em opções e dados opcionais sem necessidade também não entram. Cada caso vira `if` com retorno explícito, e dado obrigatório que falta vira erro, nunca um valor inventado. Consequências no release: `package-lock.json` e `tagprefix` obrigatórios, `--bump auto` removido (sem `--bump`, a versão sai dos commits) |
