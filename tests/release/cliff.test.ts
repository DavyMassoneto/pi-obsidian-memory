// Testa as regras reais do cliff.toml deste projeto contra o binário do git-cliff.
import { copyFileSync, readFileSync } from "node:fs";
import { join } from "node:path";
import { afterEach, describe, expect, it } from "vitest";
import { nextVersion, previewChangelog, writeChangelog } from "../../scripts/release/cliff.ts";
...
describe("nextVersion", () => {
  it("sem nenhuma tag, usa a versão inicial 0.0.1", async () => {
    ...
    expect(await nextVersion(dir, "auto", "v")).toBe("0.0.1");
  });
  ...
  it.each([
    ...
    expect(await nextVersion(repoAt("0.1.0", message), "auto", "v")).toBe(expected);
  });

  it("a partir de 1.0, quebra sobe o major", async () => {
    expect(await nextVersion(repoAt("1.2.3", "feat!: nova API"), "auto", "v")).toBe("2.0.0");
  });

  it("--bump força o incremento", async () => {
    expect(await nextVersion(repoAt("0.1.0", "docs: só documentação"), "patch", "v")).toBe("0.1.1");
  });
});
