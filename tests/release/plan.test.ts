...
import { buildPlan, NothingToRelease, type PlanInput, type ReleasePlan, type StepId } from "../../scripts/release/plan.ts";
...
describe("buildPlan", () => {
  ...
  it("fecha com a mensagem da tag e sem push automático", () => {
    expect(argsOf(buildPlan(input), "finish")).toEqual(["flow", "release", "finish", "0.2.0", "--message", "v0.2.0", "--no-push"]);
  });
  ...
  it.each(["0.1.0", "0.0.9"])("recusa quando a próxima versão (%s) não é maior que a atual", (next) => {
    expect(() => buildPlan({ ...input, next })).toThrow(NothingToRelease);
  });

  it("recusa versões inválidas", () => {
    expect(() => buildPlan({ ...input, next: "0.2" })).toThrow(/versão inválida/);
  });
});
